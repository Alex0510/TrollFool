...
struct AppListCell: View {
    ...
    @State private var keychainResultMessage: String?
    ...
    var body: some View {
        ...
        .alert(isPresented: .constant(keychainResultMessage != nil)) {
            Alert(
                title: Text("清理 Keychain"),
                message: Text(keychainResultMessage ?? ""),
                dismissButton: .default(Text("确定")) {
                    keychainResultMessage = nil
                }
            )
        }
    }
        ...
        if app.dataContainerURL != nil {
            if #available(iOS 15, *) {
                Button(role: .destructive) {
                    confirmClearKeychain()
                } label: {
                    Label("清理 Keychain", systemImage: "key")
                }
            } else {
                Button {
                    confirmClearKeychain()
    ...
    private func confirmClearKeychain() {
        let alert = UIAlertController(
            title: "清理 Keychain",
            message: "此操作将清除「\(app.name)」的所有 Keychain 数据（包括保存的密码、登录状态、设备标识等），此操作不可撤销。是否继续？",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "取消", style: .cancel))
        alert.addAction(UIAlertAction(title: "确认清理", style: .destructive) { _ in
            performClearKeychain()
        })
    ...
    private func performClearKeychain() {
        DispatchQueue.global(qos: .userInitiated).async {
            do {
                let injector = try InjectorV3(app.url)
                try injector.clearKeychainData(for: app.bid)
                DispatchQueue.main.async {
                    keychainResultMessage = "Keychain 数据已清除。"
                }
            } catch {
                DispatchQueue.main.async {
                    keychainResultMessage = "清除失败：\(error.localizedDescription)"
                }
            }
        }
    }
